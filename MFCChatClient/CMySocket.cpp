...
void	CMySocket::OnConnect(int nErrorCode) {
	...
	CString strShow = dlg->m_time.Format("%X");//格式化时间
	strShow += _T("与服务器端连接成功！");//拼接
	*/
	CString strShow = dlg->CatShowMessage(_T("与服务器端连接成功！"), _T(""));
...
void CMySocket::OnReceive(int nErrorCode) {
	...
	*/
	CString strShow = dlg->CatShowMessage(_T(" 【服务器端】："), strRecvBuffer);
	//将拼接好的缓冲区数据显示到记录列表框
	dlg->m_listBox.AddString(strShow);
	//回调父类OnReceive()函数
